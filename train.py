...
from condensed_mathematics import condensed_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    top_etfs = [{"ticker": t, "condensed_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "discrete_levels": config.DISCRETE_LEVELS,
        "profunctor_dim": config.PROFUNCTOR_DIM,
    ...
    out_file = f"output/condensed_{timestamp}.json"
