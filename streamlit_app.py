...
st.set_page_config(page_title="Condensed Mathematics", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Condensed Mathematics</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Clausen-Scholze condensed sets | Discrete-continuous interface | Profunctor cohomology</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Condensed")
...
st.sidebar.markdown(f"**Discrete levels:** {config.DISCRETE_LEVELS} | **Profunctor dim:** {config.PROFUNCTOR_DIM}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'condensed_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Cohomology rank: {etf['condensed_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Cohomology Rank"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Cohomology Rank", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Condensed Cohomology Rank (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Condensed mathematics** (Clausen-Scholze) handles large topological spaces.
        - The **discrete-continuous interface** mixes tick-level and diffusion phenomena.
        - The **condensed profunctor** is a sheaf-like structure capturing this interface.
        - The score is the **cohomology rank** – higher = more structure at the interface.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the condensed cohomology ranks.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Condensed Mathematics | Clausen-Scholze condensed sets for discrete-continuous phenomena")
